...
namespace HutongGames.PlayMaker.Actions
{
    ...
    public class DOTweenRigidbodyLookAtGameObject : FsmStateAction
    {
        ...
        {
            tweener = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DOLookAt(target.Value.transform.position, duration.Value, axisConstraint, up.Value);
            ...
            if (debugThis.Value) Debug.Log("GameObject [" + State.Fsm.GameObjectName + "] FSM [" + State.Fsm.Name + "] State [" + State.Name + "] - DOTween RigidBody Look At GameObject - SUCCESS!");
