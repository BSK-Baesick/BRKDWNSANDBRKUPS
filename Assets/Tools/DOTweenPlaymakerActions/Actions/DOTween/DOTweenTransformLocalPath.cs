...
namespace HutongGames.PlayMaker.Actions
{
    ...
    public class DOTweenTransformLocalPath : FsmStateAction
    {
        ...
        public override void OnEnter()
        {
            ...
            if (go == null)
            {
                if (debugThis.Value) Debug.Log("GameObject [" + State.Fsm.GameObjectName + "] FSM [" + State.Fsm.Name + "] State [" + State.Name + "] - DOTween Transform Local Path - ERROR: The gameObject is null");
                return;
            }

            Transform trn = go.GetComponent<Transform>();

            if (trn == null)
            {
                if (debugThis.Value) Debug.Log("GameObject [" + State.Fsm.GameObjectName + "] FSM [" + State.Fsm.Name + "] State [" + State.Name + "] - DOTween Transform Local Path - ERROR: The GameObject [" + go.name + "] does not have a Transform Component");
                return;
            }
            ...
            {
                case LookAt.nothing:
                    tweener = trn.DOLocalPath(waypoints, duration.Value, pathType, pathMode, resolution.Value, gizmoColor.Value)
            ...
            if (debugThis.Value) Debug.Log("GameObject [" + State.Fsm.GameObjectName + "] FSM [" + State.Fsm.Name + "] State [" + State.Name + "] - DOTween Transform Local Path - SUCCESS!");
