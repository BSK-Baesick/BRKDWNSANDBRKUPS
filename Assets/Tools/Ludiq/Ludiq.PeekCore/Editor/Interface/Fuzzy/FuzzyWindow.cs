	...
	public sealed class FuzzyWindow : EditorWindow
	{
		...
		private void OnOptionsGUI(float anim, FuzzyOptionNode parent)
		{
			...
			(
				...
				(tree.searchable ? LudiqStyles.searchFieldOuterHeight : 0) + Styles.headerHeight,
				position.width - 2,
				height - ((tree.searchable ? LudiqStyles.searchFieldOuterHeight : 0) + 1) - Styles.headerHeight
			);
			...
			else
			{
				...
				for (var i = 0; i < parent.children.Count; i++)
				{
					...
					if (e.type == EventType.Repaint)
					{
						using (LudiqGUI.color.Override(node.dim ? new Color(1, 1, 1, 0.66f) : Color.white))
						{
